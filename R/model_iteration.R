  ...
  function(candidate_predictors,
    ...
    independent_variable_info <- merge(
      ...
      data.frame(sum = c(
        ...
        setNames(nrow(model_dep_df), "(Intercept)")
      )),
      by.x = "variable",
      by.y = "row.names",
      all.x = T
...
collate_base_models <-
  function(candidate_variables_list,
    ...
    base_models <-
      purrr::pmap(list(candidate_variables_list, candidate_variables_df_list), function(candidate_predictors,
                                                                                        candidate_predictors_info)
        assemble_base_models(
          candidate_predictors,
          candidate_predictors_info,
          model_df,
          model_dep_df_modified,
          with_intercept,
          drop_flexi_vars,
          run_up_to_flexi_vars,
          drop_pvalue_precision,
